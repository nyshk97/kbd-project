...
> ⚠️ **2026-08-03: 未完成のまま保留にした。** 下記の目的（ZMKのビルド・書き込み・split設定、Chocソケットのはんだ付け、JLCPCB発注）は**すべて達成済み**で、残るXIAOのdirect mountはんだ付けのみ未実施。完成させない判断の理由と残る資産は `build-log.md` の「保留」節を参照。
> 以下の内容は当時の計画書としてそのまま残している。
